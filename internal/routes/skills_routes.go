...
func SetupSkillsRoutes(api fiber.Router, handler *http.SkillsMasterHandler) {
	...
	skills.Get("/recommended", handler.GetRecommendedSkills) // GET /api/v1/skills/recommended - Get recommended skills
	skills.Post("/by-ids", handler.GetSkillsByIDs)           // POST /api/v1/skills/by-ids - Get multiple skills by IDs
	skills.Get("/type/:type", handler.GetSkillsByType)       // GET /api/v1/skills/type/technical - Get by type
	skills.Get("/:id", handler.GetSkillByID)
}
